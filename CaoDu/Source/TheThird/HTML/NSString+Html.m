...
+ (NSString *)importStyleWithHtmlString:(NSString *)HTML
{
    ...
    NSString *replace = [NSString stringWithFormat:@"<html > <link href=\"%@\" rel=\"stylesheet\" type=\"text/css\"/> <head><style>img{max-width:%f !important;}</style></head> <body>",filePath,kScreenWidth - 50/PxWidth];
    //2.替换原有的命令字符串

    HTML = [NSString stringWithFormat:@"%@%@</body></html>",replace,HTML];
    //返回新配置的HTML字符串
    return HTML;
