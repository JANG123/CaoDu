...
-(void)drawTextFiel{
    ...
    [_passWordTextFiel addTarget:self action:@selector(textFieldDidChange:) forControlEvents:UIControlEventEditingChanged];
...
- (IBAction)MoneyPassWordAction:(id)sender {
    if (_passWordTextFiel.text.length < 6) {
        ...
        [[LoginRegistDateTolls shareGetLoginRegistDate]ResetFundpassWithClientId:[UserAccount shareGetUserAccountDate].aUser.ClientID NewPassword:_passWordTextFiel.text LoginRegistPassValue:^(NSString *code) {
...
- (void)textFieldDidChange:(id) sender {
    if (sender == _passWordTextFiel) {
        UITextField *field = (UITextField *)sender;
        if (field.text.length > 6) {
            NSString *tempText = [field.text substringToIndex:6];
            _passWordTextFiel.text = tempText;
        }
    }
}
